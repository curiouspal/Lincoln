...
ggplot(data=allcities, aes(Date, Home.Value)) + geom_point() + 
  theme(
    plot.title = element_text(size=20, face="bold", vjust=3), 
    axis.text.x = element_text(size=5, angle = 90, hjust = 1), 
    axis.text.y = element_text(size=18), 
...
for(i in 1:length(allcities$Home.Value)) {
  b <- as.vector(strsplit(as.character(allcities$Home.Value[i]), "$"))
  allcities$Home.Value1[i] <- b[[1]][1]
}
  
  
head(allcities$Home.Value1)
  
strsplit(allcities$Home.Value, "$", fixed=TRUE)